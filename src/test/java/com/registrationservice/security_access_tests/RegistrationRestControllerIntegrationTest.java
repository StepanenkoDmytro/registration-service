...
class RegistrationRestControllerIntegrationTest {
    ...
    @Test
    void registrationTest_WithAnonymous() throws Exception {
        SignUpDto test = new SignUpDto("invalid-token", "test@", "test");
    ...
    @Test
    void checkDecisionTest_WithAnonymous() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/registration/invalid-token"))
                .andExpect(MockMvcResultMatchers.status().isNotFound());
    }
